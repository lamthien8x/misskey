  ...
  <div class="_panel" :class="$style.list">
    ...
    <div v-else :class="$style.grid">
      <div v-for="p in items" :key="p.id" :class="$style.card">
        ...
        <div v-if="p.rewardType === 'online'" :class="$style.actions">
          <MkButton :disabled="p.hasConfirmed" @click="confirm(p)"><i class="ti ti-circle-check"></i> Đã nhận quà</MkButton>
          <MkButton :disabled="p.hasReported" warn @click="report(p)"><i class="ti ti-alert-triangle"></i> Chưa nhận được quà</MkButton>
        </div>
      </div>
    </div>
  </div>
...
async function confirm(p: any) {
  await misskeyApi('help/gift-confirm', { id: p.id });
  p.hasConfirmed = true;
}

async function report(p: any) {
  await misskeyApi('help/gift-report', { id: p.id });
  p.hasReported = true;
}
